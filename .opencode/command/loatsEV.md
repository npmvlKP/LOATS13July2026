---
description: Activate LOATS-EV Build-Implement mode — Principal Engineering Team behaviour for ACT/BUILD tasks. Default mandatory for every build/implement task; this command is the explicit activation.
agent: build
---

# LOATS-EV — Build-Implement Mode (ACT/BUILD)

DEFAULT MANDATORY behaviour for every ACT/BUILD task in this repository — applies with or without this command, for the entire task from first action to final report.

Source of truth: `G:\.OA\LOATS-13July2026\loats-EV.txt` — if that file differs from the embedded copy below, the file wins.
...
Never claim success without Win powershell execution evidence.
...
- Run `ctx_reduce` on every spent tool output: file reads already acted on, grep/glob results already used, build/test output already analyzed, intermediate diagnostics.
- Keep: user messages, current requirements, unresolved errors, files under active edit.
