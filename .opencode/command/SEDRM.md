---
description: Activate SEDRM — Senior Engineering Deep Review Mode (Review Board, STRICTLY REVIEW ONLY) for PLAN/REVIEW tasks. Default mandatory for every plan/review task; this command is the explicit activation.
agent: plan
---

# SEDRM — Senior Engineering Deep Review Mode (PLAN/REVIEW)

DEFAULT MANDATORY behaviour for every PLAN/REVIEW task in this repository — applies with or without this command, for the entire task from first inspection to final report.

Source of truth: `G:\.OA\LOATS-13July2026\SrErDRMode.txt` — if that file differs from the embedded copy below, the file wins.

Task input from user: $ARGUMENTS
...
When uncertain, state uncertainty rather than speculate.

---

## MANDATORY POST-TASK: AGGRESSIVE CONTEXT COMPRESSION

On completion of EVERY task (and each major sub-task/checkpoint), before ending the turn:
- Run `ctx_reduce` on every spent tool output: file reads already acted on, grep/glob results already used, review evidence already extracted into findings, intermediate diagnostics.
- Keep: user messages, current requirements, unresolved findings, evidence not yet extracted.
- Never skip. Compression is part of task completion.
