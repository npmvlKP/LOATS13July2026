    ...
    try:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if i == 1 and line.strip().startswith("from unittest.mock import patch"):
                ...
                modified_lines.append(
                    line.rstrip() + "  # noqa: E402 - patch import for test isolation\n"
                )
                continue

            if in_malformed_section:
                if i == 4 and line.strip().startswith(
                    "from datetime import UTC, datetime"
                ):
                    modified_lines.append(
                        line.rstrip() + "  # noqa: E402 - import after docstring\n"
                    )
                ...
                elif i == 17 and line.strip().startswith(
...
def main():
    ...
    files_to_fix = [
        (
            ...
            add_noqa_to_database_async_additions,
        ),
        (
        ...
        (
            project_root / "tests" / "test_orchestrator.py",
            add_noqa_to_test_orchestrator,
        ),
        (
            project_root / "tests" / "test_trailing_stop.py",
            add_noqa_to_test_trailing_stop,
        ),
    ]
