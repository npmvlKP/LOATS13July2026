...
CHECKS: list[Check] = [
    ...
    Check(
        "L04",
        "Trailing Stop Runtime",
        True,
        [PY, "-m", "pytest", "tests/test_trailing_stop_runtime.py", "-q", "--tb=short"],
        30,
    ),
    Check(
        "L05",
        "Audit Dual-Write",
        True,
        [PY, "-m", "pytest", "tests/test_audit_dual_write.py", "-q", "--tb=short"],
        30,
    ),
    ...
    Check(
        ...
        "Queue Backpressure",
        True,
        [PY, "scripts/probe_l08_queue_backpressure.py"],
        30,
    ),
    # GATE
    Check(
...
def run(check: Check) -> Result:
    ...
    try:
        ...
        return Result(
            check,
            Status.FAIL if check.critical else Status.WARN,
            dur,
            p.stdout,
            p.stderr,
        )
...
def main() -> int:
    ...
    for r in results:
        ...
        print(
            f"  {sym[r.status]:4} | {r.check.id:<6} | {r.check.name:<40} | {tag:<10} | {r.duration:.1f}s"
        )
    ...
    print(f"\n{sep}")
    print(
        f"  CRITICAL: {cp}/{len(crit)} PASS  |  INFO WARNINGS: {iw}  |  {total_dur:.1f}s total"
    )
    print(f"{sep}\n")
