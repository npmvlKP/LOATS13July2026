...
def check_coverage_thresholds(module_coverage: List[Tuple[str, float, int]], threshold: float = 80.0) -> Tuple[bool, List[str]]:
...
def main():
    ...
    for module_name, percent_covered, missing_lines in sorted(module_coverage, key=lambda x: x[1]):
        status = "[PASS]" if percent_covered >= threshold else "[WARN]"
        print(f"{status} {module_name}.py: {percent_covered:.1f}% ({missing_lines} statements missed)")
