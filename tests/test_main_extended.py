...
async def test_trading_system_run_once_failure(trading_system):
    with patch("src.loats.main.scheduler.run_ta_scan", side_effect=Exception("Scan error")):
...
async def test_handle_shutdown_signal(trading_system):
    with patch.object(trading_system, 'shutdown', new_callable=AsyncMock) as mock_shutdown:
        await trading_system._handle_shutdown_signal(signal.SIGINT)
        mock_shutdown.assert_called_once()

@pytest.mark.asyncio
async def test_main_function_success():
    with patch("src.loats.main.TradingSystem.initialize", new_callable=AsyncMock) as mock_init, \
         patch("src.loats.main.TradingSystem.start", new_callable=AsyncMock) as mock_start:
        await main()
...
async def test_main_function_failure():
    with patch("src.loats.main.TradingSystem.initialize", side_effect=Exception("Main error")), \
...
async def test_start_already_running(trading_system):
    ...
    with patch("src.loats.main.alerts.start", new_callable=AsyncMock) as mock_alerts_start:
        await trading_system.start()
