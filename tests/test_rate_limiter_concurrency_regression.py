...
class TestRateLimiterConcurrencyRegression:
    ...
    async def test_order_rate_limiter_concurrency(self) -> None:
        ...
        # Should have approximately 3 successful acquisitions (max_ops from singleton default)
        # Allow for small timing variations in concurrent scenarios (2-4 range)
        successful = sum(results)
        assert 2 <= successful <= 4, (
            f"Expected 2-4 successful acquisitions, got {successful}"
        )
    ...
    async def test_mixed_rate_limiter_concurrency(self) -> None:
        ...
        # Should have approximately 3 successful order acquisitions (from singleton default)
        # Allow for small timing variations in concurrent scenarios (2-4 range)
        order_successful = sum(results[:50])
        assert 2 <= order_successful <= 4, (
            f"Expected 2-4 successful order acquisitions, got {order_successful}"
        )

        # Should have approximately 3 successful smart order acquisitions (from singleton default)
        # Allow for small timing variations in concurrent scenarios (2-4 range)
        smart_successful = sum(results[50:])
        assert 2 <= smart_successful <= 4, (
            f"Expected 2-4 successful smart order acquisitions, got {smart_successful}"
        )
