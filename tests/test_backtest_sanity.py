...
def make_ohlc(symbol, base, count, mins=5):
    ...
    return [HistoricalData(symbol=symbol, timestamp=now - timedelta(minutes=mins*(count-1-i)), open=base+i*0.5-1, high=base+i*0.5+2, low=base+i*0.5-2, close=base+i*0.5, volume=10000+i*100, interval=f'{mins}min') for i in range(count)]
