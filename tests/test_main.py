...
async def test_trading_system_initialization(trading_system):
    ...
    with patch("src.loats.main.db._initialize_database") as mock_db_init, \
         patch("src.loats.main.db.verify_audit_log_integrity", return_value=True) as mock_db_verify, \
...
async def test_trading_system_start_shutdown(trading_system):
    with patch("src.loats.main.alerts.start", new_callable=AsyncMock) as mock_alerts_start, \
         ...
         patch("src.loats.main.TradingSystem._wait_for_shutdown", new_callable=AsyncMock):
        ...
        with patch("src.loats.main.scheduler.shutdown", new_callable=AsyncMock) as mock_scheduler_shutdown, \
...
async def test_trading_system_run_once(trading_system):
    with patch("src.loats.main.scheduler.run_ta_scan", new_callable=AsyncMock) as mock_ta, \
