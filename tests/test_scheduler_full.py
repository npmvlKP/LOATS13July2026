import datetime
import unittest.mock import AsyncMock, MagicMock, patch
import pytest
...
async def test_run_ta_scan_success(scheduler):
    with patch("src.loats.scheduler.async_client", new_callable=AsyncMock) as mock_client, \
        ...
        mock_client.get_history.return_value = {
            "data": [
                {"timestamp": "2026-07-19T10:00:00", "open": 100, "high": 110, "low": 90, "close": 105, "volume": 1000}
            ]
        }
        mock_client.get_quotes.return_value = {
            "data": {"NSE:NIFTY50": {"last_price": 105, "open": 100, "high": 110, "low": 90, "close": 105, "volume": 1000}}
        }
...
async def test_run_signal_generation_success(scheduler):
    with patch("src.loats.scheduler.async_client", new_callable=AsyncMock) as mock_client, \
        ...
        mock_db.async_get_latest_signals = AsyncMock(return_value=[MagicMock(strength=0.8, indicators={})])
...
async def test_check_market_status_open(scheduler):
    ...
    assert scheduler.scheduler.add_job.call_count > 0
