"""
Simple test to verify HTML escaping is working correctly.
"""
import pytest
...
@pytest.mark.asyncio
async def test_html_escaping_simple():
    """Simple test to verify HTML escaping is working."""
    ...
    with patch.object(AlertSystem, 'send_alert', new_callable=AsyncMock) as mock_send_alert:
        ...
        # Check what's actually in the message
        if "<script>test</script>" in message:
            print("✅ HTML escaping is working correctly!")
            assert True
        elif "<script>test</script>" in message:
            print("❌ HTML escaping is NOT working - raw HTML found!")
            assert False, "HTML escaping is not working"
        else:
            print("❓ Unexpected message content")
            assert False, f"Unexpected message content: {message}"
