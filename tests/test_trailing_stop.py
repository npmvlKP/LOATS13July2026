...
class TestInitializeTrailingStop:
    def test_fixed_long(self):
        c = _cfg(
            TrailingStopType.FIXED, TransactionType.BUY, 18500.0, {"fixed_amount": 75.0}
        )
    ...
    def test_volatility_long(self):
        c = _cfg(
            TrailingStopType.VOLATILITY,
            TransactionType.BUY,
            18500.0,
            {"volatility": 0.02, "multiplier": 2.0},
        )
...
class TestDeadCodePaths:
    ...
    def test_percentage_long_adjusts(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.PERCENTAGE, TransactionType.BUY, 1000.0, percentage=0.02
        )
    ...
    def test_percentage_short_adjusts(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.PERCENTAGE, TransactionType.SELL, 1000.0, percentage=0.02
        )
        with patch("loats.trailing_stop.datetime") as mdt:
            mdt.datetime.now.return_value = __import__("datetime").datetime.now(
                __import__("datetime").timezone.utc
    ...
    def test_atr_long_adjusts(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.ATR, TransactionType.BUY, 1000.0, atr=10.0, multiplier=2.0
        )
        with patch("loats.trailing_stop.datetime") as mdt:
            mdt.datetime.now.return_value = __import__("datetime").datetime.now(
                __import__("datetime").timezone.utc
    ...
    def test_atr_short_adjusts(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.ATR, TransactionType.SELL, 1000.0, atr=10.0, multiplier=2.0
        )
        with patch("loats.trailing_stop.datetime") as mdt:
            mdt.datetime.now.return_value = __import__("datetime").datetime.now(
                __import__("datetime").timezone.utc
    ...
    def test_ratchet_discrete_long(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.RATCHET, TransactionType.BUY, 1000.0, percentage=0.02
        )
        with patch("loats.trailing_stop.datetime") as mdt:
            mdt.datetime.now.return_value = __import__("datetime").datetime.now(
                __import__("datetime").timezone.utc
    ...
    def test_ratchet_discrete_short(self):
        e = TrailingStopEngine()
        c = self._make_cfg(
            TrailingStopType.RATCHET, TransactionType.SELL, 1000.0, percentage=0.02
        )
        with patch("loats.trailing_stop.datetime") as mdt:
            mdt.datetime.now.return_value = __import__("datetime").datetime.now(
                __import__("datetime").timezone.utc
