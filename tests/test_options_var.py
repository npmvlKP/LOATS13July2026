...
class TestVaRNormal:
    ...
    def test_parametric_custom(self):
        ...
        v = calculate_parametric_var(rets, confidence_level=0.95, mean=0.0, std_dev=0.01)
...
class TestGreeksEdge:
    def test_zero_vol_call(self):
        g = OptionsEngine().calculate_greeks(S=18000, K=18000, t=30/365, sigma=0.0, option_type=OptionType.CALL)
        assert isinstance(g, Greeks)

    def test_zero_vol_put(self):
        g = OptionsEngine().calculate_greeks(S=18000, K=18000, t=30/365, sigma=0.0, option_type=OptionType.PUT)
        assert isinstance(g, Greeks)

    def test_expired_call_itm(self):
        g = OptionsEngine().calculate_greeks(S=18500, K=18000, t=0.0, sigma=0.25, option_type=OptionType.CALL, allow_expired=True)
        assert g.delta == 1.0
        assert g.gamma == 0.0

    def test_expired_call_otm(self):
        g = OptionsEngine().calculate_greeks(S=17500, K=18000, t=0.0, sigma=0.25, option_type=OptionType.CALL, allow_expired=True)
        assert g.delta == 0.0

    def test_expired_put_itm(self):
        g = OptionsEngine().calculate_greeks(S=17500, K=18000, t=0.0, sigma=0.25, option_type=OptionType.PUT, allow_expired=True)
        assert g.delta == -1.0

    def test_expired_put_otm(self):
        g = OptionsEngine().calculate_greeks(S=18500, K=18000, t=0.0, sigma=0.25, option_type=OptionType.PUT, allow_expired=True)
        assert g.delta == 0.0

    def test_expired_error_attrs(self):
        try:
            OptionsEngine().calculate_greeks(S=18000, K=18000, t=-0.5, sigma=0.25, option_type=OptionType.CALL)
    ...
    def test_standalone_expired_raises(self):
        with pytest.raises(ExpiredContractError):
            calculate_greeks(S=100, K=100, t=-0.1, r=0.05, sigma=0.2, option_type=OptionType.CALL)

    def test_standalone_fallback_call(self):
        with patch("loats.options.delta", side_effect=ZeroDivisionError):
            g = calculate_greeks(S=18000, K=18000, t=30/365, r=0.05, sigma=0.25, option_type=OptionType.CALL)
            assert g.delta == 0.0

    def test_standalone_fallback_put(self):
        with patch("loats.options.delta", side_effect=ZeroDivisionError):
            g = calculate_greeks(S=18000, K=18000, t=30/365, r=0.05, sigma=0.25, option_type=OptionType.PUT)
            assert g.delta == 0.0

class TestVaREdge:
    def test_historical_empty(self):
        with pytest.raises(ValueError): calculate_var([], 0.95)
...
class TestPortfolioVaR:
    ...
    def test_none_current_price(self):
        try:
            r = calculate_portfolio_var([Trade(symbol="NIFTY", quantity=10, entry_price=100.0, entry_time=datetime.now(UTC), transaction_type=TransactionType.BUY, status="OPEN")], 0.95)
            assert r.var_value == 0.0
    ...
    def test_valid_trade(self):
        try:
            r = calculate_portfolio_var([Trade(symbol="NIFTY", quantity=10, entry_price=100.0, entry_time=datetime.now(UTC), exit_price=105.0, exit_time=datetime.now(UTC), transaction_type=TransactionType.BUY, pnl=50.0, status="CLOSED")], 0.95)
...
class TestOptionPortfolioVaR:
    ...
    def test_with_contracts(self):
        ...
        c = [OptionContract(symbol="NIFTY23JAN18000CE", strike_price=18000.0, expiry=expiry, option_type=OptionType.CALL, last_price=150.0, open_interest=10000, volume=5000, implied_volatility=0.25, delta=0.5, gamma=0.02, quantity=2)]
...
class TestComprehensiveVaR:
    def test_full(self):
        ...
        expiry = datetime(2030, 1, 26, 15, 30, tzinfo=UTC)
        trades = [Trade(symbol="NIFTY", quantity=10, entry_price=100.0, entry_time=datetime.now(UTC), exit_price=105.0, exit_time=datetime.now(UTC), transaction_type=TransactionType.BUY, pnl=50.0, status="CLOSED")]
        contracts = [OptionContract(symbol="CE", strike_price=18000.0, expiry=expiry, option_type=OptionType.CALL, last_price=150.0, open_interest=10000, volume=5000, implied_volatility=0.25, delta=0.5, gamma=0.02, quantity=2)]
    ...
    def test_custom_confidence(self):
        try:
            with patch("loats.options.datetime") as mdt:
                mdt.datetime.now.return_value = datetime.now(UTC)
                mdt.UTC = UTC
                r = calculate_comprehensive_var_analysis([], [], [], confidence_levels=[0.90, 0.99])
...
class TestIVEdge:
    def test_expired_raises(self):
        with pytest.raises(ExpiredContractError):
            OptionsEngine().calculate_implied_volatility(price=150, S=18000, K=18000, t=-0.1, option_type=OptionType.CALL)

    def test_standalone_expired_raises(self):
        with pytest.raises(ExpiredContractError):
            calculate_implied_volatility(price=150.0, S=18000.0, K=18000.0, t=-0.1, r=0.05, option_type=OptionType.CALL)

    def test_call_oob(self):
        assert OptionsEngine().calculate_implied_volatility(price=20000, S=18000, K=18000, t=30/365, r=0.05, option_type=OptionType.CALL) == 0.2

    def test_put_oob(self):
        assert OptionsEngine().calculate_implied_volatility(price=20000, S=18000, K=18000, t=30/365, r=0.05, option_type=OptionType.PUT) == 0.2

    def test_standalone_numerical_fallback(self):
        with patch("loats.options.implied_volatility", side_effect=ValueError("fail")):
            assert calculate_implied_volatility(price=150.0, S=18000.0, K=18000.0, t=30/365, r=0.05, option_type=OptionType.CALL) == 0.2

class TestOptionChain:
    def test_analyze_chain(self):
        expiry = datetime(2030, 1, 26, 15, 30, tzinfo=UTC)
        c = [OptionContract(symbol="CE", strike_price=18000, expiry=expiry, option_type=OptionType.CALL, last_price=150, open_interest=10000, volume=5000, implied_volatility=0.25, delta=0.5, gamma=0.02, theta=-0.05, vega=0.1, rho=0.03, quantity=1)]
    ...
    def test_unexpected_error(self):
        ...
        bad.symbol = "X"; bad.strike_price = 18000; bad.expiry = expiry
        bad.option_type = OptionType.CALL; bad.last_price = 150
        bad.open_interest = 10000; bad.volume = 5000; bad.implied_volatility = None
...
class TestBSAndSmile:
    def test_call(self):
        assert OptionsEngine().calculate_black_scholes(S=100, K=100, t=1, sigma=0.2, option_type=OptionType.CALL) > 0

    def test_put(self):
        assert OptionsEngine().calculate_black_scholes(S=100, K=100, t=1, sigma=0.2, option_type=OptionType.PUT) > 0

    def test_smile_empty(self):
        assert OptionsEngine().calculate_volatility_smile([], 18000) == []

    def test_parity(self):
        r = OptionsEngine().calculate_put_call_parity(call_price=10, put_price=5, S=100, K=100, t=30/365, r=0.05)
...
class TestOptionsAnalysis:
    ...
    def test_analyze_dict(self):
        chain = {"expiry_dates": ["2023-01-26"], "options": [{"strike_price": 18000, "option_type": "CE", "open_interest": 10000, "implied_volatility": 0.25, "last_price": 150}, {"strike_price": 18000, "option_type": "PE", "open_interest": 12000, "implied_volatility": 0.26, "last_price": 140}]}
        r = OptionsAnalysis().analyze_option_chain(chain, 18000)
        assert "atm_strike" in r
        assert "oi_analysis" in r

class TestSingletons:
    def test_options(self): assert isinstance(options, OptionsEngine)
    def test_analysis(self): assert isinstance(analysis, OptionsAnalysis)
...
class TestGreeksFallbacks:
    def test_calculate_greeks_numerical_error_call(self):
        ...
        with patch('loats.options.delta', side_effect=ZeroDivisionError('div0')):
            g = e.calculate_greeks(S=18000, K=18000, t=0.1, r=0.05, sigma=0.25, option_type=OptionType.CALL)
    ...
    def test_calculate_greeks_numerical_error_put(self):
        ...
        with patch('loats.options.delta', side_effect=ValueError('bad')):
            g = e.calculate_greeks(S=18000, K=18000, t=0.1, r=0.05, sigma=0.25, option_type=OptionType.PUT)
        assert g.delta == 0.0  # S == K, so S < K is False, delta=0.0

    def test_standalone_greeks_call(self):
        g = calculate_greeks(S=18000, K=18000, t=0.1, r=0.05, sigma=0.25, option_type=OptionType.CALL)
        assert isinstance(g, Greeks)
    ...
    def test_standalone_greeks_error_reraise(self):
        with patch('loats.options.delta', side_effect=RuntimeError('unexpected')):
            with pytest.raises(RuntimeError, match='unexpected'):
                calculate_greeks(S=18000, K=18000, t=0.1, r=0.05, sigma=0.25, option_type=OptionType.CALL)
    ...
    def test_standalone_iv_error_reraise(self):
        with patch('loats.options.implied_volatility', side_effect=RuntimeError('bad')):
            with pytest.raises(RuntimeError, match='bad'):
                calculate_implied_volatility(price=150, S=18000, K=18000, t=0.1, r=0.05, option_type=OptionType.CALL)
...
class TestIVFallback:
    def test_iv_fallback_brentq(self):
        e = OptionsEngine()
        with patch('loats.options.implied_volatility', side_effect=ValueError('fail')):
            iv = e.calculate_implied_volatility(price=150, S=18000, K=18000, t=0.1, option_type=OptionType.CALL)
        assert iv > 0
...
class TestAnalyzeChainErrors:
    def test_analyze_chain_numerical_error(self):
        e = OptionsEngine()
        expiry = datetime(2030, 1, 26, 15, 30, tzinfo=UTC)
        c = OptionContract(symbol='CE', strike_price=18000.0, expiry=expiry, option_type=OptionType.CALL,
                           last_price=150.0, open_interest=10000, volume=5000, implied_volatility=1e10,
                           delta=0.5, gamma=0.02, quantity=1)
...
class TestVolatilitySmile:
    def test_smile_with_none_iv(self):
        e = OptionsEngine()
        expiry = datetime(2030, 1, 26, 15, 30, tzinfo=UTC)
        c1 = OptionContract(symbol='CE1', strike_price=17000.0, expiry=expiry, option_type=OptionType.CALL,
                            last_price=200.0, open_interest=100, volume=50, implied_volatility=0.25,
                            delta=0.8, gamma=0.01, quantity=1)
        c2 = OptionContract(symbol='CE2', strike_price=19000.0, expiry=expiry, option_type=OptionType.CALL,
                            last_price=50.0, open_interest=200, volume=100, implied_volatility=None,
                            delta=0.2, gamma=0.01, quantity=1)
...
class TestPortfolioGreeks:
    def test_portfolio_greeks_single(self):
        ...
        expiry = datetime(2030, 1, 26, 15, 30, tzinfo=UTC)
        c = OptionContract(symbol='CE', strike_price=18000.0, expiry=expiry, option_type=OptionType.CALL,
                           last_price=150.0, open_interest=10000, volume=5000, implied_volatility=0.25,
                           delta=0.5, gamma=0.02, quantity=2)
...
class TestBSClamped:
    def test_bs_clamps_small_t(self):
        e = OptionsEngine()
        price = e.calculate_black_scholes(S=18000, K=18000, t=1e-6, sigma=0.25, option_type=OptionType.CALL)
        assert price > 0
