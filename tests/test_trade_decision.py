...
from loats.models import (
    FundsData, HistoricalData, Signal, SignalType,
    Trade, TradeDecision, TransactionType,
)
...
def fixture_historical_data():
    ...
    return [
        HistoricalData(
            ...
            open=24500.0 + i * 10, high=24550.0 + i * 10,
            low=24470.0 + i * 10, close=24510.0 + i * 10,
            volume=1000000 + i * 10000, interval="5min",
        ) for i in range(30)
    ]
...
def _make_signals(count, strength_base=0.75, signal_type=SignalType.BUY):
    ...
    return [
        Signal(
            symbol="NIFTY", signal_type=signal_type,
...
class TestCompositeStrengthCalculation:
    ...
    def test_composite_strength_with_opposing_signals(self, strength_engine):
        now = datetime.now(UTC)
        signals = [
            Signal(symbol="NIFTY", signal_type=SignalType.BUY, strength=0.8,
                timestamp=now, indicators={}, confidence=0.8,
                metadata={"source": StrengthSource.TECHNICAL_ANALYSIS.value}),
            Signal(symbol="NIFTY", signal_type=SignalType.BUY, strength=0.75,
                timestamp=now - timedelta(seconds=30), indicators={}, confidence=0.75,
                metadata={"source": StrengthSource.SENTIMENT.value}),
            Signal(symbol="NIFTY", signal_type=SignalType.SELL, strength=0.6,
                timestamp=now - timedelta(seconds=60), indicators={}, confidence=0.6,
...
class TestSignalSourceValidation:
    ...
    def test_validate_rejects_unknown_source(self, strength_engine):
        ...
        signals = _make_signals(2) + [
            Signal(symbol="NIFTY", signal_type=SignalType.BUY, strength=0.68,
                timestamp=now - timedelta(seconds=60), indicators={}, confidence=0.72,
...
class TestTradeDecisionCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_decision_valid_signals(self, trade_decision_engine, fixture_historical_data, fixture_funds):
        with patch("loats.trade_decision.rules_engine") as mock_rules:
            ...
            decision, _ = await trade_decision_engine.create_trade_decision(
                signals=_make_signals(3), historical_data=fixture_historical_data,
                current_price=24500.0, funds=fixture_funds, current_positions=[])
    ...
    @pytest.mark.asyncio
    async def test_create_decision_rejects_weak_signals(self, trade_decision_engine, fixture_historical_data, fixture_funds):
        with patch("loats.trade_decision.rules_engine") as mock_rules:
            mock_rules.apply_gating_rules.return_value = (True, {"reason": "gating_passed", "iv_rank": 50.0, "adx": 30.0, "vix": 14.0})
            mock_rules.check_position_limits.return_value = (True, {"reason": "within_limits"})
            mock_rules.session_state = "REGULAR"
            decision, r = await trade_decision_engine.create_trade_decision(
                signals=_make_signals(3, 0.2), historical_data=fixture_historical_data,
                current_price=24500.0, funds=fixture_funds, current_positions=[])
        assert decision is None
        assert r["reason"] == "insufficient_strength"

    @pytest.mark.asyncio
    async def test_create_decision_rejects_invalid_sources(self, trade_decision_engine, fixture_historical_data, fixture_funds):
        now = datetime.now(UTC)
        signals = _make_signals(2) + [
            Signal(symbol="NIFTY", signal_type=SignalType.BUY, strength=0.68,
                timestamp=now - timedelta(seconds=60), indicators={}, confidence=0.72,
        ...
        decision, r = await trade_decision_engine.create_trade_decision(
            signals=signals, historical_data=fixture_historical_data,
            current_price=24500.0, funds=fixture_funds, current_positions=[])
        assert decision is None
