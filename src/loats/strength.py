...
class StrengthEngine:
    ...
    def calculate_composite_strength(
        self,
        signals: List[Signal],
        require_opposition_gate: bool = True
        ...
        for source, signal_list in source_signals.items():
            ...
            source_strength = self.calculate_source_strength(strongest_signal, source_enum)
    ...
    def check_opposition_gate(
        ...
        for source, signals in source_signals.items():
            ...
            if (primary_direction == "BUY" and strongest_signal.signal_type == SignalType.SELL) or \
               (primary_direction == "SELL" and strongest_signal.signal_type == SignalType.BUY):
    ...
    def validate_signal_sources(
        ...
        diversity_score = self.calculate_strength_diversity({s: signals for s in source_set})
    ...
    def get_source_strength_breakdown(
        self,
        signals: List[Signal]
    ) -> Dict[str, Any]:
        """
        ...
        if breakdown["total_weight"] > 0:
            breakdown["composite_strength"] = breakdown["total_strength"] / breakdown["total_weight"]

        breakdown["composite_strength"] = self.normalize_strength(breakdown["composite_strength"])
