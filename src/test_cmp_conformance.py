...
def test_cmp_rule_1_nifty_lot_size():
...
def test_cmp_rule_2_no_resting_time():
...
def test_cmp_rule_3_algo_id_tagging():
...
def test_cmp_rule_4_ops_threshold():
...
async def test_cmp_rule_4_rate_limiter_integration(setup_env):
...
def test_cmp_rule_5_paper_trading_analyzer_mode():
...
def test_cmp_rule_6_trailing_sl_and_sl_m():
    """CMP Rule 6: Bot-logic trailing SL + SL-M."""
    from src.loats.models import OrderType, Order
...
def test_cmp_rule_11_position_limits():
    """CMP Rule 11: Position limits 5 NIFTY / 3 BANKNIFTY."""
    settings = get_settings()

    # Verify NIFTY position limit
    assert settings.max_nifty_positions == 5, \
        f"CMP Rule 11 violated: max_nifty_positions={settings.max_nifty_positions} should be 5"

    # Verify BANKNIFTY position limit
    assert settings.max_banknifty_positions == 3, \
        f"CMP Rule 11 violated: max_banknifty_positions={settings.max_banknifty_positions} should be 3"

def test_cmp_rule_12_trailing_sl_m():
...
def test_cmp_singleton_pattern():
...
def test_cmp_validation_logic():
    ...
    assert cmp.validate({"symbol1": 10, "symbol2": 15}) is True, "Should allow when max under limit"
