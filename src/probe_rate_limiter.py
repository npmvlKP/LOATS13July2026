...
def main() -> None:
    ...
    print(
        f"   - Singleton identity check: "
        f"{smart_limiter is get_smart_order_rate_limiter()}"
    )

    # Test actual rate limiting behavior
    print("\n4. Testing Actual Rate Limiting Behavior:")

    async def test_order_limiter() -> int:
    ...
    async def test_smart_limiter() -> int:
    ...
    print(
        f"   - Order limiter allowed: {order_acquired}/10 "
        f"(expected: {settings.max_ops})"
    )
    print(
        f"   - Smart limiter allowed: {smart_acquired}/10 "
        f"(expected: {settings.max_ops})"
    )
    ...
    print(
        f"   - Order limiter enforcement: "
        f"{'PASS' if order_acquired == settings.max_ops else 'FAIL'}"
    )
    print(
        f"   - Smart limiter enforcement: "
        f"{'PASS' if smart_acquired == settings.max_ops else 'FAIL'}"
    )

    # Overall compliance
    compliant = (
        settings.max_ops <= 10
        and settings.max_ops <= 3
        and settings.max_ops == 3
        and order_acquired == settings.max_ops
        and smart_acquired == settings.max_ops
    )

    print(
        f"\n6. Overall CMP Rule 4 Compliance: "
        f"{'FULLY COMPLIANT' if compliant else 'NON-COMPLIANT'}"
    )
