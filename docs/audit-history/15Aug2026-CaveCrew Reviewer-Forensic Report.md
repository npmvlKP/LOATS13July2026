# LOATS13July2026 — Forensic Engineering Report (Review #6R — CaveCrew Reviewer Verification & Final Consolidation)
...
**Role of this pass:** This is the Reviewer counterpart to the same-day Investigator report (FR6, `15Aug2026-CaveCrewInvestigator-Forensic Report.md`). It **independently re-verified every FR6 finding with live evidence**, re-ran the full quality-gate battery and the rate-limiter probe, spot-checked the CMP conformance claims at source level, and consolidates the seven-review chain (FR1 15Jul → FR6 15Aug) into one final deliverable.

**Evidence basis (all live, this pass, clean venv, HEAD `36d0c52`):**
`git rev-parse/log/status`; empirical rate-limiter probe (singleton identity, effective cap, 100-acquire burst); `check_deps_sync.py` PASS; `ruff check` **FAIL — 135 errors** (101×E501, 17×RUF003, 9×RUF001, 8×RUF002); `ruff format --check` **FAIL — 20 files**; `isort --check-only` **FAIL — 11 files**; `flake8` PASS; `mypy --strict` PASS (27 files); `bandit` PASS; `pytest` **843 passed / 0 failed / 9 warnings, 116.60s, coverage 80.43%** (gate ≥80 met); per-module coverage table; source reads of `orchestrator.py`, `rate_limiter.py`, `settings.py`, `openalgo.py` call sites, `database.py:655`, `cache.py:23-29`, `strike_selection.py`, `check_per_module_coverage.py`, `Dockerfile`, `pyproject.toml`; git inventory (302 tracked files; 38 root-level `.py`; 38 root-level `.md`); grep sweeps confirming CMP-scope absences (no `rules.py`, no `strength.py`, no ADX/BBANDS/CCI/Hurst/regime, no `TradeDecision`, no order-modification counter, no `date.today()`).
...
### 1.2 CMP demand vs build reality (verified this pass)
...
| Dimension | FR4 (01Aug) | FR5 (08Aug) | FR6 Investigator (15Aug) | **FR6-R Reviewer (15Aug, this)** | Trend |
|---|---|---|---|---|---|
| Tests | 325/14 fail | 640/0 | 843/0 (1 flaky seen) | **843/0 (flaky NOT reproduced this run)** | ✅ Up |
...
└── utils/
    ├── cache.py                        # TTLCache (asyncio.Lock); stray optional redis import (L6R-2)
...
**Order path (financial-critical), verified line-level this pass:**
`place_order` → `_async_check_kill_switch()` → `get_order_rate_limiter().acquire()` (`openalgo.py:799` — **default 50/s, F6-C-01**) → CB open-check (fail-fast) → `_request(..., idempotency_key=...)` → `Idempotency-Key` header (`openalgo.py:245,605`) + DB duplicate-order guard. Kill-switch block writes an audit entry. Smart-order path identical at `openalgo.py:876`.
...
| redis | ❌ | ❌ | 🟡 Not a dependency, but `utils/cache.py:23-29` retains a guarded optional `import redis.asyncio` + unused `REDIS_AVAILABLE` — dead LITE-drift code |

`scripts/check_deps_sync.py` runs as a CI gate and **PASSES (re-run this pass)** — manifest drift is now mechanically prevented.

---

## 5. Module-by-Module Review

Coverage figures from this pass's live run (843 tests, branch coverage on):
...
| `database_async_additions.py` | 227 | **31%** | 🔴 Defects | Production-preferred DB path, least-tested code; aiosqlite thread teardown exceptions observed in this run (`PytestUnhandledThreadExceptionWarning`) |
...
- **Issue ID:** F6-C-01 (FR6 investigator; **confirmed by this pass**)
- **Category:** Compliance / Financial Safety
- **Severity:** Critical · **Confidence:** Certain (empirical, reproduced this pass)
- **Owning specialists:** Production Debugging Engineer, Security Auditor, Reliability Engineer
- **Evidence (this pass, 2026-08-15, clean venv, HEAD `36d0c52`):**
  - Probe output (verbatim):
    ...
    ```
  - `src/loats/utils/rate_limiter.py` — all four factory functions (`get_order_rate_limiter` :355, `get_smart_order_rate_limiter` :389, plus sync variants :421/:455) hard-code `max_ops=50` when no argument is supplied ("No `max_ops` → return the *process-wide* default singleton (50 ops/sec)").
  ...
  - **Root-cause refinement (new, this pass):** the limiter **class constructors already default correctly** — `rate_limiter.py:35/139/254`: `self.max_ops = max_ops if max_ops is not None else settings.max_ops`. The factories defeat this by explicitly passing `max_ops=50`. The fix is therefore one line per factory: pass through `None`/`settings.max_ops` instead of `50`.
...
- **Suggested resolution (pending approval):** In each factory default `max_ops` from `get_settings().max_ops` (or pass `None` and let the constructor default apply); add a regression test asserting `get_order_rate_limiter().max_ops == settings.max_ops` and a CI assertion that the effective cap ≤ 10.
...
- **Issue ID:** F6-C-02 (FR6 investigator; **confirmed by this pass — exact same counts**)
- **Category:** Process / Quality Gates
- **Severity:** Critical (blocks merge; false history) · **Confidence:** Certain (live runs)
- **Evidence (this pass):**
  - `ruff check src/ tests/ scripts/` → **135 errors** (statistics: 101×E501 line-too-long, 17×RUF003, 9×RUF001, 8×RUF002 ambiguous-unicode).
...
- **Impact / consequences:** Red CI; false confidence from `git log`; process controls (CONTRIBUTING.md, `commit_message_check.py`) exist and are routinely violated by the very commits that add them.
- **Risk assessment:** Critical (process) — the project cannot merge or release from this HEAD.
- **Suggested resolution:** `ruff check --fix` + `ruff format` + `isort`; normalize the 34 ambiguous-unicode literals; re-run all gates to green; enable branch protection requiring CI on `main`; enforce the existing commit-message hook.
...
- **Issue ID:** F6-H-03 (FR6; **confirmed** — coverage table + live pytest warnings)
- **Category:** Reliability / Testing · **Severity:** High · **Confidence:** Certain
- **Evidence (this pass):** `database_async_additions.py` **31%** (miss lines 33-300+: the true-async bodies); pytest run emits `PytestUnhandledThreadExceptionWarning` from aiosqlite worker threads ("Event loop is closed"); `database.py:1788` pool `maxsize=10`; async methods dispatch aiosqlite-pool → `to_thread` fallback while legacy sqlite3 thread-locals persist.
...
- **Impact:** Production-preferred DB path is the least-tested code; closed-loop exceptions indicate pool connections outliving test (and potentially runtime) loops; three persistence mechanisms multiply failure modes (retention/locking/audit-ordering differ per path).
- **Possible consequences:** Post-shutdown data loss windows, WAL contention, silent divergence between DB and JSONL audit ordering across paths.
- **Risk assessment:** High. **Suggested resolution:** (a) fix pool lifecycle (close joins worker threads; `pool.close()` in `TradingSystem.shutdown()`), (b) raise additions-module coverage ≥80% or fold into `database.py`, (c) document the dispatch precedence contract. **Complexity:** Medium (1 day). **Priority:** **P1**.

### 🟠 F6-H-04 — CMP strategy core absent (rules / strength / sizing / trailing / VaR / session / Analyzer routing)

- **Issue ID:** F6-H-04 (FR6; **confirmed by grep + module inventory this pass**)
- **Category:** Scope / Architecture · **Severity:** High · **Confidence:** Certain
- **Evidence (this pass):** `src/loats/**` contains 25 `.py` files — **no `rules.py`, no `strength.py`**; grep for `(?i)(ADX|BBANDS|Bollinger|CCI|Hurst|regime)` → zero substantive hits; no `TradeDecision` anywhere; no order-modification counter (rule 7); `trailing_stop_loss` appears only as a stored/passed field (models/DB/payload_builder) — no ratchet engine; `max_position_size=1000` / `max_position_per_symbol=1000` vs CMP rule 11 (5 NIFTY / 3 BANKNIFTY); orchestrator combines (ta+sentiment)/2 — only 2 sources, no opposition gate, no ≥3-source requirement; no VaR computation in `options.py`/`ta.py`.
...
- **Impact:** The system cannot trade the CMP strategy; orchestrator "signals" are 2-source averages stored to DB, not gated `TradeDecision`s routed to Analyzer.
- **Risk assessment:** High (scope) — this is the difference between a research scaffold and the mandated trading system.
- **Suggested resolution:** Execute CMP P4/P5 as a program: `rules.py` gates (IV-rank>40/ADX<25/VIX>15 sell; inverse buy) → `strength.py` ≥3-source composite with opposition gate → 2% fixed-frac cost+margin-aware sizing → monotonic trailing ratchet with SL-M → per-source circuit breakers → session lifecycle (PRE_OPEN→REGULAR→POST_CLOSE) → `TradeDecision` routed to Analyzer; plus rule 7 modification counter and rule 11 position limits.
...
### 🟠 F6-H-05 — Orchestrator correctness defects (7, all source-verified this pass)

- **Issue ID:** F6-H-05 (FR6; **all seven re-verified line-level**)
- **Category:** Correctness / Reliability · **Severity:** High · **Confidence:** Certain
- **Evidence (`src/loats/orchestrator.py`, this pass):**
...
- **Impact:** Wrong stats, import fragility, task-loss risk, alert floods, divide-by-zero on a realistic broker state, budget violations.
- **Suggested resolution:** Single increment site; lazy settings; hold strong task ref + done-callback; real drain (await the task with timeout after setting the event); alert backoff (e.g. ≥1/min); guard `available_margin == 0`; move TA inside the parallel budget with its own `wait_for`. Add regression tests per defect. **Complexity:** Low-Medium (½ day). **Priority:** **P1**.

---

## 8. Medium Priority Findings

> Format: ID · Category · Severity · Confidence · Evidence → Resolution · Complexity · Priority. All verified this pass unless noted.
...
- **🟡 F6-M-02 — Flaky rate-limiter test** — Testing · Medium · Medium. Investigator observed `tests/test_rate_limiter.py::TestAsyncRateLimiter::test_get_wait_time` fail in a full-suite `-x` run yet pass isolated; **this pass's full run: 843/0 — not reproduced**. Timing/window sensitivity remains the probable cause. → De-flake by injecting a clock; add retry-stable assertions. P2 · Low.
...
- **🟡 F6-M-06 — Repo hygiene regression (quantified worse this pass)** — Hygiene · Medium · Certain. 302 tracked files; **38 root-level `.py`** debug/verify scaffolds; **38 root-level `.md`** reports (FR6 said "20+"; actual 38). Ruff `exclude` list names 9 of the root scripts (normalizing the mess rather than removing it). → `git rm` scaffolds/reports (or move to `docs/audit-history/`), extend `.gitignore`, then shrink the ruff exclude list. P2 · Low.
...
- **🟢 F6-L-01 — 34 ambiguous-unicode literals** (RUF001-003: smart quotes/en-dashes in strings/docstrings/comments). Byte-scan (FR6): 0×U+FFFD in `src/` — cosmetic, not corruption (R5-F-21 stays refuted). Fold into F6-C-02 cleanup. P3.
...
- **🟢 F6-L-03 — Empty-match pytest.warns** — `tests/test_orchestrator.py` `pytest.raises(..., match="")` — PytestWarning observed live this run: "matching against an empty string will *always* pass". Assert a real substring. P3.
- **🟢 F6-L-04 — AsyncMock never awaited** — `tests/test_scheduler_coverage.py` RuntimeWarnings observed live this run: mock misuse, coroutines dropped. Fix mocks. P3.
...
| Check | Status | Evidence (this pass where applicable) |
|---|---|---|
...
## 15. Code Quality Review (live, HEAD `36d0c52`, this pass)
...
| bandit | ✅ PASS (exit 0) |
| pip-audit | ✅ PASS (FR6 run; no known vulns, local pkg skipped) |
...
- **843 tests / 0 failed** this pass (FR6 saw one flaky failure: `test_get_wait_time` — not reproduced here; still suspect, F6-M-02).
...
- Live-observed warnings this run: aiosqlite thread `PytestUnhandledThreadExceptionWarning` (F6-H-03); empty-match `pytest.raises` (F6-L-03); `AsyncMock` never awaited ×3 (F6-L-04).
...
## Appendix B — Investigator (FR6) Findings Disposition — all independently re-verified this pass
...
| F6-M-02 | 🟡 Confirmed-suspect | Investigator saw 1 failure; this pass's full run 843/0 — intermittent, consistent with timing sensitivity |
...
| F6-M-06 | ✅ Confirmed, **worse** | 302 tracked; root `.py` = 38; root `.md` = **38** (FR6 said "20+") |
| F6-M-07 | ✅ Confirmed | `Dockerfile:43,46` |
| F6-L-01…07 | ✅ Confirmed (L-03/L-04 reproduced live in pytest warnings) | — |

**Zero refutations. Zero material contradictions. One quantification update (M-06) and one root-cause refinement (C-01).** The Investigator's FR6 report stands as accurate.
...
# F6-C-01 reproduction (probe run this pass; see report §6 for verbatim output)
