...
dec_used = [
    f.name
    for f in files
    if "from decimal import" in f.read_text(encoding="utf-8")
    or "import decimal" in f.read_text(encoding="utf-8")
]
...
aware = [
    f.name
    for f in files
    if "datetime.now(tz" in f.read_text(encoding="utf-8")
    or "datetime.now(timezone" in f.read_text(encoding="utf-8")
    or "timezone.utc" in f.read_text(encoding="utf-8")
]
...
kill = [
    f.name
    for f in files
    if "kill_switch" in f.read_text(encoding="utf-8")
    or "kill switch" in f.read_text(encoding="utf-8").lower()
]
...
struct = [
    f.name
    for f in files
    if "jsonlogger" in f.read_text(encoding="utf-8").lower()
    or "JsonFormatter" in f.read_text(encoding="utf-8")
]
