...
surfaces re-pinned in lockstep (428, then 429 once the discharge evidence
below was tracked — the wave had also swept in a stray benchmark run
artifact) and a lockstep regression test added. Follow-up (same day): the
leniency branch itself was removed — any failed check now fails the
verdict — and the regression test pins that outcome.

## 5. Discharge evidence made clone-reproducible (04Sep2026, follow-up)

`reports/p1_analyze_latency_20260904_040609.json` — the genuine 100/100
live TCS round-trip artifact that discharges F8-L-03/P1 — was whitelisted
in `.gitignore` and added to the canonical-evidence set in
`verify_f8c02_external.py` check 8, so HC-29 (and the P1 verdict) can be
re-verified from a fresh clone. Benchmark run artifacts under
`reports/performance/` are gitignored (regenerated by
`scripts/benchmark_performance.py`; a stray run artifact had been swept
into the wave commit and was untracked).
